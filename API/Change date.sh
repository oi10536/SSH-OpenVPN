#! / bin / bash
tput setaf 7; tput setab 4; tput bold; printf '% 33s% s% -12s \ n' "เปลี่ยนวันหมดอายุ"; tput sgr0
echo ""
...
awk -F : '$3 >= 500 { print $1 }' /etc/passwd | grep -v '^nobody' | while read user
  do
	xpire="$(chage -l $user | grep -E "Account expires" | cut -d ' ' -f3-)"
	if [[ $expire == "never" ]]
	then
		never = "Never"
		printf '% -30s% s \ n' "$ user" "Never"
	else
		...
		then
			...
			datanormal = "$ (date -d" $ expire "'+% d /% m /% Y')"
			printf '% -30s% s \ n' "$ user" "$ datanormal"
		fi
	fi
  done
tput sgr0
...
read -p "ชื่อผู้ใช้เพื่อเปลี่ยนวันหมดอายุ:" ผู้ใช้
if [[-z $ user]]
then
	echo ""
	...
	then
		...
		sysdate = "$ (echo" $ inputdate "| awk -v FS = / -V OFS = - '{พิมพ์ $ 3, $ 2, $ 1}')
		...
		then
			...
			then
				...
				then
					วันนี้ = "$ (วัน - วันนี้ +"% Y% m% d ")"
					timemachine = "$ (วันที่ -d" $ sysdate "+"% Y% m% d ")"
					then [$ วันนี้ --ge $ timemachine]
					then
						echo ""
